...
/* Horizontal 2:1 chroma for packed RGBF32 → 4:2:0 (NV12, etc.). Without this,
 * chrSrcHSubSample=1 still calls the full converter with width/2, so UV is
 * taken only from the left half of the frame. */
                                                 ...
                                                 int width, int is_be, int32_t *rgb2yuv)
{
    ...
    for (i = 0; i < width; i++) {
        int r = (lrintf(av_clipf(65535.0f * rdpx(&src[6 * i]),     0.0f, 65535.0f)) +
